...
void ImGuiManager::RenderFrame(GuiRenderContext& renderContext)
{
    ...
    for (int iCommandList = 0; iCommandList < imGuiDrawData->CmdListsCount; ++iCommandList)
    {
        ...
        if (gRenderSystem.mDynamicVertexCache.Allocate(eBufferContent_Vertices, cmd_list->VtxBuffer.size_in_bytes(), cmd_list->VtxBuffer.Data, vBuffer))
        {
        ...
        if (gRenderSystem.mDynamicVertexCache.Allocate(eBufferContent_Indices, cmd_list->IdxBuffer.size_in_bytes(), cmd_list->IdxBuffer.Data, iBuffer))
        {
